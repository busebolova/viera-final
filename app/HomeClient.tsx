...
export function HomeClient() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/github/content?file=home&t=${timestamp}`)
        .then((r) => r.json())
        .catch(() => ({ data: null })),
      fetch(`/api/github/content?file=projects&t=${timestamp}`)
        .then((r) => r.json())
        .catch(() => ({ data: null })),
      fetch(`/api/github/content?file=contact&t=${timestamp}`)
        .then((r) => r.json())
        .catch(() => ({ data: null })),
    ])
      .then(([homeRes, projRes, contRes]) => {
        console.log("[v0] Home data from GitHub:", homeRes.data)
        console.log("[v0] Projects data from GitHub:", projRes.data)
        console.log("[v0] Contact data from GitHub:", contRes.data)

        if (homeRes.data && Object.keys(homeRes.data).length > 0) {
          setContent({ ...DEFAULT_CONTENT, ...homeRes.data })
        }
        if (projRes.data && Object.keys(projRes.data).length > 0) {
          setProjects({ ...DEFAULT_PROJECTS, ...projRes.data })
        }
        if (contRes.data && Object.keys(contRes.data).length > 0) {
          setContact({ ...DEFAULT_CONTACT, ...contRes.data })
        }
        setLoading(false)
      })
      .catch((err) => {
        console.log("[v0] Error fetching content:", err)
        setLoading(false)
      })
  }, [])

  const heroData = content.hero || content.video || DEFAULT_CONTENT.hero
